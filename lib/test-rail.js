...
class TestRail {
    ...
    constructor(options) {
        ...
        this._validate(options, 'sectionId');
     ...
     * @return {*} API response
     */
    addCase(title, content) {
        return this._post(`add_case/${this.options.sectionId}`, {
            "title": title,
            "custom_status": 1,
     ...
     * @return {*} API response
     */
    getTestsForSection(projectId, suiteId, sectionId) {
        return this._get(`get_cases/${projectId}&suite_id=${suiteId}&section_id=${sectionId}`);
    }
    ...
    publish(description, results, runners) {
        ...
        let cases = this.getTestsForSection(this.options.projectId, this.options.suiteId, this.options.sectionId)
        ...
        runners.forEach((runner, runnerIdx) => {   
            ...
            resultByRunner = results.filter(r => {
                return JSON.stringify(r.runner) === JSON.stringify(runner);
            })
    ...
    pushCase(suite, contents, caseId) {
        if (!this.options.pushCase) return

        // 1. get all case
        let availCases = this.getTestsForSection(this.options.projectId, this.options.suiteId, this.options.sectionId)
        availCases = availCases.map(c => c.id)

        // 2a. no this case id: add case and update id (manually)
        if (typeof caseId === 'undefined' || !availCases.includes(caseId)) {
            // newCase = this.addCase(suite.title, contents.join('\n'))
            // console.warn(`add test case : ${newCase.id}`)
        } else {
            // 2b. case id match: check automation type
            let nowCase = this.getCase(caseId)
            if (nowCase.custom_automation_type !== AutomationType.implemented) return

            // 3. get case contents and update
            this.updateCase(
                caseId, suite.title, contents.join('\n')
            )
            console.log(`update test case : ${caseId}`)
        }
    }
}
