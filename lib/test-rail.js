...
class TestRail {
     ...
     * @private
     */
    async _request(method, api, body, error = undefined) {
        let options = {
            headers: {
        ...
        let result;
        result = await request(method, this._url(`/api/v2/${api}`), options);
        console.log('====================================!');
        console.log(JSON.parse(result.getBody('utf8')));
        console.log('====================================!');
        return JSON.parse(result.getBody('utf8'));
        // try {
        //     result = await request(method, this._url(`/api/v2/${api}`), options).getBody('utf8').then(JSON.parse);
        // } catch (e) {
     ...
     */
    async publish(name, description, results, runners, callback = undefined) {
        let runs = [];
        let body = null;
        let plan = null
        if (typeof this.options.suiteId !== 'number') {
            if (this.options.updatePlan) {
                ...
                plan = await this.getPlan(this.options.updatePlan);
                plan.entries.forEach(async entry => {

                    const suiteInfo = await this.getSuite(entry.runs[0].suite_id);
                    const currentCases = await this.getTestsForRun(entry.runs[0].id).map(c => c.case_id);
                    let resp = await this.getTestsForSuite(this.options.projectId, suiteInfo.id);
                    suiteInfo.cases = resp.map(c => c.id);
                    suiteInfo.newCases = results.filter(r => suiteInfo.cases.includes(r.case_id));

                    //reset case id listing
                    await this.updateTestPlanEntry(
                        plan.id,
                        entry.id,
                    ...
                    body = [];
                    //add new results
                    body.push(await this.addResultsForCases(entry.runs[0].id, suiteInfo.newCases));
                })
            } else {
                let testPlanEntries = []
                    //1. create the test plan

                plan = await this.addPlan(name, description, []);

                runners.forEach(async(runner, runnerIdx) => {
                    await this.options.suiteId.forEach(async(suiteId, suiteIdx) => {
                        let suiteInfo = await this.getSuite(suiteId);

                        let resp = await this.getTestsForSuite(this.options.projectId, suiteId);

                        suiteInfo.cases = resp.map(c => c.id);

                        resp = await this.addTestPlanEntry(
                            plan.id,
                            suiteId,
                        ...
                        testPlanEntries.push(resp.runs[0]);
            ...
            if (this.options.updateRun) {
                ...
                await this.addCasesToRun(runs[0].id, results.map(r => r.case_id));

                //update results to a run
                await this.addResultsForCases(runs[0].id, results)
            } else {
                // console.log('runners')
                // console.log(runners)
                runners.forEach(async(runner, idx) => {
                    const runName = `${name} - (${runner.deviceName || runner.platform} - ${runner.browserName})`
                    runs.push(await this.addRun(runName, description, this.options.suiteId, results.filter(result => {
                        return JSON.stringify(result.runner) === JSON.stringify(runner);
                    }).map(result => result.case_id)));
                    await this.addResultsForCases(runs[idx].id, results);
                })
            }
