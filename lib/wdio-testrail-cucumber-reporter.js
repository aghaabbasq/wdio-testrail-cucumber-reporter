...
class WdioTestRailCucumberReporter extends events.EventEmitter {
    ...
    constructor(baseReporter, config) {
        ...
        this.on('test:pending', (test) => {
            ...
            let caseIds = Utils.titleToCaseIds(Array.from(test.tags).map(x => x.name).join(', '));
        ...
        this.on('test:pass', (test) => {
            ...
            let caseIds = Utils.titleToCaseIds(Array.from(test.tags).map(x => x.name).join(', ')); //look in the parent for the testrail id (represents a scenario or scenario outline)
            const runnerKey = Object.keys(test.runner)[0];
            if (caseIds.length > 0) {
                let results = caseIds.map(caseId => {
                    return {
        ...
        this.on('end', async(data) => {
            if (process.env.REPORT == 'true') {
                ...
                const description = `${name}
													...
													`;
                try {
                    await this.testRail.publish(name, description, this._results, data.capabilities);
                } catch (error) {
                    console.error('Error while pushng data');
                    throw error;
                }

            } else {
                console.warn(`Reporting environment variable is set to [REPORT= ${process.env.REPORT}] so results will not be pushed to TestRail.`);
            }
        });
    }
}
