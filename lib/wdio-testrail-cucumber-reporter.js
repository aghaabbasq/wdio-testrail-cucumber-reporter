    ...
    constructor(baseReporter, config) {
        ...
        this.on('test:pending', (test) => {
            ...
            if (typeof caseId !== 'undefined') {
                if (this._results[caseId]) {
                    this._results[caseId].comment = dedent`${this._results[caseId].comment}
                                                           ${test.title}: pending`
                } else {
                    this._results[caseId] = {
                        case_id: caseId,
                        status_id: this._results[caseId].status_id,
        ...
        this.on('test:pass', (test) => {
            ...
            if (typeof caseId !== 'undefined') {
                if (this._results[caseId] && this._results[caseId].status_id === Status.Passed) {
                    this._results[caseId].comment = dedent`${this._results[caseId].comment}
                                                           ${test.title}: pass`
                } else {
                    this._results[caseId] = {
        ...
        this.on('test:fail', (test) => {
            ...
            if (typeof caseId !== 'undefined') {
                if (this._results[caseId]) {
                    this._results[caseId].status_id = Status.Failed
                    this._results[caseId].comment = dedent`${this._results[caseId].comment}                                 
                                                           ${test.title}: fail
                                                               ${test.err.message}
                                                               ${test.err.stack}`
        ...
        this.on('end', (data) => {
            ...
            let results = [];
            for (let [key, value] of Object.entries(this._results)) {
                results.push(value);
            }
