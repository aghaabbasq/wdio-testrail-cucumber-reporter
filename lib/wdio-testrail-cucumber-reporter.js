	...
	constructor(baseReporter, config) {
		...
		this.on('suite:start', (suite) => {
			if (suite.parent && suite.parent != null) {
				console.log(`\n\n${chalk.yellow.underline.bold('Starting Scenario: '+suite.title)}`);
			} else {
				console.log(`${chalk.blue.underline('- Executing Feature: '+suite.title)}`);
			}
		});
		...
		this.on('end', () => {
			if (process.env.REPORT == 'true') {
				...
				console.log(`Reporting environment variable is set to [${process.env.REPORT}] so results will not be pushed to TestRail.`);
			}
		});
	}
}
